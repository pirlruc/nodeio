from inspect import signature, isclass, isabstract
...
class NodeFactory(Factory):
    ...
    def register(self, key: key_str, functor: Callable) -> Self:
        ...
        :raises TypeError: If the return annotation associated with the functor is not a BaseNode 
        """
        # Although this validation is performed there is no guarantee that the create method will return a BaseNode.
        # This is only guaranteed in the create method.
        return_values = signature(functor).return_annotation
        if return_values != Self and (not isclass(return_values) or isabstract(return_values) or not isinstance(return_values(),BaseNode)):
            error_message = f'Functor return annotation provided an incorrect type for key {key}. Please review functor annotation'
    ...
    def create(self, key: key_str, *args, **kwargs) -> BaseNode:
        """
        Returns a node created by the functor registered with the provided key.
        ...
        :raises TypeError: If the node created by the functor is not a BaseNode.
        """
        result = super().create(key, *args, **kwargs)
        if not isinstance(result,BaseNode):
            error_message = f'Functor for key {key} returns an incorrect type: {type(result)}. Please review functor'
            NodeIOLogger().logger.error(error_message)
            raise TypeError(error_message)
        return result
