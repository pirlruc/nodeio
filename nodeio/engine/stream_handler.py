...
class StreamHandler(BaseModel, validate_assignment = True, arbitrary_types_allowed=True):
    ...
    def add_output_stream(self, stream: OutputStream, origin: key_str) -> Self:
        ...
        if stream.key in self.__output.keys():
            error_message = f'Output stream {stream.key} already exists. Please review stream key'
            NodeIOLogger().logger.error(error_message)
            raise KeyError(error_message)            
        self.__output[stream.key] = self.__Connection(origin=origin, stream=stream)
        return self
    ...
    def register_connection(self, key: key_str, ending: key_str) -> Self:
        ...
        :param key: Key of the output stream to register the connection with an input stream.
        :type key: key_str
        ...
        if ending == origin_node:
            error_message = f'Origin node {origin_node} and ending node {ending} for stream {key} is the same. Please review configuration'
            NodeIOLogger().logger.error(error_message)
    ...
    def has_unconnected_streams(self) -> bool:
        ...
        :return: Returns true if there any unconnected output streams. If all output streams are connected, returns false.
        :rtype: bool
        """
