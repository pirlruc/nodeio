...
class TestInputStream(unittest.TestCase):
    ...
    def test_constructor(self):
        ...
        stream_2 = InputStream(
            arg=InputArg(key='b'),
            stream=OutputStream(key='b'),
            actions=[ListAction(value=0)],
        )
    ...
    def test_complete_input_arg(self):
        ...
        stream_2 = InputStream(
            arg=InputArg(key='a'),
            stream=OutputStream(key='a', type=int),
            actions=[ListAction(value=1)],
        )
    ...
    def test_from_configuration(self):
        ...
        config_2 = nodeio.engine.configuration.InputStream(
            ...
            stream='d',
            actions=[
                nodeio.engine.configuration.DictAction(key='e'),
                nodeio.engine.configuration.ListAction(index=1),
            ],
        )
