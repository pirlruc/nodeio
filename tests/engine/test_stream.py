...
class TestInputStream(unittest.TestCase):
    def test_invalid_constructor(self):
        with self.assertRaises(pydantic_core.ValidationError):
            ...
            InputStream(arg=InputArg(key="a"),
                        stream=OutputStream(key="a"), actions=[1])

    def test_constructor(self):
        stream_1 = InputStream(arg=InputArg(
            key="a"), stream=OutputStream(key="a"))
        stream_2 = InputStream(arg=InputArg(key="b"), stream=OutputStream(
            key="b"), actions=[ListAction(value=0)])
    ...
    def test_invalid_update(self):
        stream = InputStream(arg=InputArg(key="a"),
                             stream=OutputStream(key="a"))
        with self.assertRaises(pydantic_core.ValidationError):
            stream.arg = 1
            stream.stream = 1
            stream.actions = 1
    ...
    def test_complete_input_arg(self):
        stream_1 = InputStream(arg=InputArg(
            key="a"), stream=OutputStream(key="a", type=int))
        stream_2 = InputStream(arg=InputArg(key="a"), stream=OutputStream(
            key="a", type=int), actions=[ListAction(value=1)])
        self.assertEqual(stream_1.arg.type, int)
        self.assertIsNone(stream_2.arg.type)

    def test_complete_output_stream(self):
        stream_1 = InputStream(arg=InputArg(
            key="a", type=int), stream=OutputStream(key="a"))
        stream_2 = InputStream(arg=InputArg(key="a", type=int), stream=OutputStream(
            key="a"), actions=[ListAction(value=1)])
    ...
    def test_invalid_data_types(self):
        InputStream(arg=InputArg(key="a", type=int), stream=OutputStream(
            key="a", type=bool), actions=[ListAction(value=1)])
        with self.assertRaises(TypeError):
            InputStream(arg=InputArg(key="a", type=int),
                        stream=OutputStream(key="a", type=bool))

    def test_valid_data_types(self):
        stream_1 = InputStream(arg=InputArg(
            key="a", type=int), stream=OutputStream(key="a", type=int))
        stream_2 = InputStream(arg=InputArg(key="a", type=int), stream=OutputStream(
            key="a"), actions=[ListAction(value=1)])
        self.assertEqual(stream_1.arg.type, int)
        self.assertEqual(stream_1.stream.type, int)
    ...
    def test_from_configuration(self):
        ...
        stream_1: InputStream = InputStream.from_configuration(
        ...
        stream_2: InputStream = InputStream.from_configuration(
...
class TestContextStream(unittest.TestCase):
    ...
    def test_from_output_stream(self):
        stream: ContextStream = ContextStream.from_output_stream(
            OutputStream(key="b", type=int))
