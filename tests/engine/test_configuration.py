...
class TestInputStream(unittest.TestCase):
    ...
    def test_json(self):
        stream_1 = InputStream(arg='a', stream='b')
        stream_2 = InputStream(
            arg='c',
            stream='d',
            actions=[DictAction(key='e'), ListAction(index=1)],
        )
        self.assertEqual(
            InputStream.model_validate_json(stream_1.model_dump_json()),
            stream_1,
        )
        self.assertEqual(
            InputStream.model_validate_json(stream_2.model_dump_json()),
            stream_2,
        )
...
class TestNode(unittest.TestCase):
    ...
    def test_constructor(self):
        ...
        node_3 = Node(
            node='d',
            input_streams=[InputStream(arg='5', stream='6')],
            output_stream='2',
        )
        ...
        self.assertEqual(node_1.input_streams, [])
        self.assertEqual(node_1.output_stream, '2')
        self.assertEqual(node_1.options, {})
        ...
        self.assertEqual(node_2.options, {})
    ...
    def test_json(self):
        ...
        node_2_ori = Node(
            node='e',
            input_streams=[InputStream(arg='1', stream='2')],
            output_stream='3',
            options={'4': 'test'},
        )
        print(node_1_ori)
...
class TestGraph(unittest.TestCase):
    ...
    def test_no_input_streams(self):
        with self.assertRaises(ConfigurationError):
            Graph(
                nodes=[{'node': 'a', 'output_stream': 'a', 'type': 'node'}],
                input_streams=[],
                output_streams=['c'],
            )
    ...
    def test_constructor(self):
        graph_1 = Graph(
            nodes=[{'node': 'a', 'type': 'node', 'output_stream': '1'}],
            input_streams=['b'],
            output_streams=['c'],
        )
        graph_2 = Graph(
            nodes=[
                {'node': 'a', 'type': 'node', 'output_stream': '1'},
                Node(node='b', output_stream='2'),
            ],
            input_streams=['b', 'd'],
            output_streams=['c', 'e'],
        )
